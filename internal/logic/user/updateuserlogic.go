...
func (l *UpdateUserLogic) UpdateUser(req *types.UpdateUserReq) (resp *types.Response, err error) {
	...
	currentUser, err = l.svcCtx.SysUserModel.SelectOneDetail(l.ctx, userId)
	if err != nil {
	...
	if currentUser.RoleID != common.SUPER_ADMIN_ROLE_ID {
	...
	if req.PasswordExpireTime != "" {
		...
		if parseErr != nil {
			...
			return &types.Response{
				...
				Msg:  "密码过期时间格式错误，请使用 yyyy-MM-dd HH:mm:ss 格式",
			}, nil
		}
