...
func (m *defaultSysRoleModel) Delete(ctx context.Context, roleId string) error {
	query := fmt.Sprintf("update %s set `deleted_at` = NOW()  where `role_id` = ? and `deleted_at` is null", m.table)
...
func (m *defaultSysRoleModel) Insert(ctx context.Context, data *SysRole) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.RoleId, data.RoleName, data.RoleDesc)
	return ret, err
}
...
func (m *defaultSysRoleModel) Update(ctx context.Context, newData *SysRole) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.RoleName, newData.RoleDesc, newData.RoleId)
	return err
}
