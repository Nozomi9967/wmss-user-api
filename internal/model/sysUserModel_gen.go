...
func (m *defaultSysUserModel) FindOne(ctx context.Context, userId string) (*SysUser, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `deleted_at` is null limit 1", sysUserRows, m.table)
	var resp SysUser
...
func (m *defaultSysUserModel) Insert(ctx context.Context, data *SysUser) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.UserId, data.UserName, data.RealName, data.Password, data.RoleId, data.Department, data.Position, data.ContactPhone, data.UserStatus, data.LastLoginTime, data.PasswordExpireTime)
	return ret, err
}
